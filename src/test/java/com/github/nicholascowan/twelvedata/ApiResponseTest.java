...
public class ApiResponseTest {
  ...
  void testTimeSeriesResponseProperties() throws Exception {
    ...
    assertEquals(
        "America/New_York",
        meta.get("exchange_timezone").asText(),
        "Exchange timezone should be America/New_York");
  ...
  void testQuoteResponseProperties() throws Exception {
    ...
    assertTrue(
        fiftyTwoWeek.has("low_change_percent"), "52-week should have 'low_change_percent' field");
    assertTrue(
        fiftyTwoWeek.has("high_change_percent"), "52-week should have 'high_change_percent' field");
  ...
  void testTimeSeriesCsvResponse() throws Exception {
    ...
    for (int i = 1; i < lines.length; i++) {
      ...
      if (!line.isEmpty()) {
        ...
        assertTrue(
            fields[0].matches("\\d{4}-\\d{2}-\\d{2}( \\d{2}:\\d{2}:\\d{2})?"),
            "Datetime should be in YYYY-MM-DD or YYYY-MM-DD HH:MM:SS format");

        // Validate numeric fields
        for (int j = 1; j < 5; j++) {
          assertTrue(
              fields[j].matches("\\d+\\.\\d+"), "Price field " + j + " should be a decimal number");
        }
  ...
  void testGeneratedUrl() {
    ...
    assertTrue(
        url.startsWith("https://api.twelvedata.com/time_series"),
  ...
  void testTimeSeriesDifferentIntervals() throws Exception {
    ...
    for (String interval : intervals) {
      ...
      assertEquals(
          "ok", response.get("status").asText(), "Status should be ok for interval: " + interval);

      if (response.has("meta")) {
        JsonNode meta = response.get("meta");
        assertEquals(
            interval, meta.get("interval").asText(), "Interval should match for: " + interval);
      }
    }
  }
  ...
  void testDifferentSymbols() throws Exception {
    ...
    assertEquals(
        "ok", tsResponse.get("status").asText(), "Time series status should be ok for: " + symbol);
